...
class Pianoroll(object):
    ...
    * pitchwheel        -> pw
    ...
    def __init__(self, sheet, ticks_per_beat):
        ...
        self.used_controls = list({cc['cn'] for cc in sum(self.control_changes, [])})
        for k in range(16):
        ...
        print(f'used controls: {self.used_controls}')
    ...
    def draw(self, height=8, aspect=4, dpi=36, nn_interval=(48, 72), tick_interval=(0, 960*4), splits_per_beat=4, channel=0, is_drum=False):
        # [01] pianoroll meta params
        fontsize = 45*height/(length(*nn_interval)+8)
        velocity_h = 5
        ...
        if is_drum:
            ...
            aspect_kbd = 1
        width = height*(aspect_kbd+aspect*self.max_tick/self.ticks_per_beat)/(top-bottom)
        keyboard_w = width*aspect_kbd*self.ticks_per_beat/(aspect*self.max_tick)*((self.max_tick+self.ticks_per_beat)/width)*4
        left = tick_interval[0] - keyboard_w
        right = tick_interval[1]
        tmp = [msg['tpo'] for msg in self.set_tempos]
        bpm_range = (round(tempo2bpm(max(tmp))), round(tempo2bpm(min(tmp))))
        ...
        ccs0 = dict.fromkeys(self.used_controls, [])
        ccs = dict.fromkeys(self.used_controls, [])
        for cn in self.used_controls:
            ccs0[cn] = [[msg['stt'], msg['value']] for msg in self.control_changes[channel] if msg['cn'] == cn]
            k_left = len(ccs0[cn])-1-[cc[0]<=tick_interval[0] for cc in ccs0[cn]][::-1].index(True)
            k_right = len(ccs0[cn])-1-[cc[0]<=tick_interval[1] for cc in ccs0[cn]][::-1].index(True)
            ccs[cn] = [[tick_interval[0], ccs0[cn][k_left][1]]] + \
                      [cc for cc in ccs0[cn] if tick_interval[0]<cc[0]<tick_interval[1]] + \
                      [[tick_interval[1], ccs0[cn][k_right][1]]]
        ...
        plt.plot(sts_stts, sts_tpos, color='#ee5511', lw=1.0, solid_capstyle='butt', solid_joinstyle='bevel', zorder=2.9, label='tempo')
        ...
        ccs_stts = dict.fromkeys(self.used_controls, [])
        ccs_values = dict.fromkeys(self.used_controls, [])
        for i, cn in enumerate(self.used_controls):
            ccs_stts[cn] = [cc[0] for cc in ccs[cn]]
            ccs_values[cn] = [cc[1]/128*length(*nn_interval)+nn_interval[0] for cc in ccs[cn]]
            plt.plot(ccs_stts[cn], ccs_values[cn], color=hsv_shader(i, 1, len(self.used_controls)+1),
                     lw=1.0, ls='--', solid_capstyle='butt', solid_joinstyle='bevel', zorder=2.9, label=f'cc{cn}')
        ...
        if plot_type == 'piecewise':
            plt.plot(pws_stts, pws_pits, color='#1155ee', lw=1.0, solid_capstyle='butt', solid_joinstyle='bevel', zorder=2.9, label='pitchwheel')
        elif plot_type == 'stair':
            plt.step(pws_stts, pws_pits, color='#1155ee', lw=1.0, solid_capstyle='butt', zorder=2.9, label='pitchwheel')
        else: pass
