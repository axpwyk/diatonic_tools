...
class Guitar(object):
    def __init__(self):
        ...
        # self._fretboard[:, :, 0]: note
        self._fretboard[:, :, 0] %= 12
        # self._fretboard[:, :, 1]: group
        self._fretboard[:, :, 1] //= 12
        # initialize pressed, accidentals and br357t
        self._initialize()
    ...
    def _initialize(self):
        ...
        self.br357t = np.zeros((25, 6), np.chararray)
    ...
    def draw_guitar(self, height=5, hue=0.0, low=-1, high=24, finger_offset=0, radius_offset=0, wh_ratio=610/44):
        ...
        # add -1 fret position (x)
        frets = [-h/5] + frets
        # delta position of frets (Δx)
        ...
        # if guitar fretboard figure not exists, draw a new one; otherwise add notes
        if not plt.fignum_exists(1):
            ...
            # draw horizontal lines i.e. strings
    ...
    def add_notes(self, notes):
        for i, note in enumerate(notes.get_notes()):
            ...
            self.br357t[indices] = notes._br357t[i]
